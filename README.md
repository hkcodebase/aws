# AWS Topics

#### [EC2 - Elastic Cloud Compute](./ec2/README.md)
#### [ELB - Elastic Load Balancing](./elb/README.md)
#### [RDS - Relational Database Service](./rds/README.md)
#### [AWS Aurora - Managed DB Service](./rds/aurora.md)
#### [ElastiCache](./rds/elasticache.md)
#### [VPC](vpc.md)
#### [Amazon S3 - Simple Storage Service](./s3/s3.md)



## Medium Articles 
 - [EC2 - Create and connect via SSH](https://medium.com/@hkcodeblogs/aws-ec2-create-and-connect-to-instance-via-ssh-354a0c1909f)
 - [S3 - Create First Bucket](https://medium.com/@hkcodeblogs/aws-s3-create-a-bucket-33a2685c9206)
 - [Create Access Keys](https://medium.com/@hkcodeblogs/aws-access-keys-f75f796e8c8)
 - [CLI setup ](https://medium.com/@hkcodeblogs/aws-cli-connect-to-aws-using-command-line-41925af062bd)
 - [Application Load Balancer](https://medium.com/@hkcodeblogs/scaling-with-aws-1-application-load-balancer-alb-b9ea2edb5f46)